...
function conformconf_settings(){
    ...
    if ( $page_name or $existing_page ){
        ...
        if ( $user_error ){
            ...
            if ( ! wp_verify_nonce( $_POST['cfcnonce'], 'cfcn-' . $user_ID ) ) {
                echo '<br/><div class="error" style="margin-left:1px;color:red"><p>Invalid or expired security token!</p></div>';
            } else {
                $iframe_link = $_POST['iframe_url'];

                // Replace spaces so URL validates
                $iframe_link = str_replace(' ', '%20', $iframe_link);

                if (filter_var($iframe_link, FILTER_VALIDATE_URL) === FALSE or !preg_match("/^https:\/\/i.cx\/.+/", $iframe_link)) {
                    echo '<br/><div class="error" style="margin-left:1px;color:red"><p>Invalid iFrame Content!</p></div>';
    ...
    $nonce = wp_create_nonce( 'cfcn-' . $user_ID );
    $replacements['NONCE'] = "<input type='hidden' name='cfcnonce' value='$nonce'/>";
